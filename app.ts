...
async function main() {
	...
	const llmRepository = createLLMRepository(db);

	const llmService = createLLMService(llmRepository);
	const mapsService = createMapsService();

	const llmController = createLLMController(llmService, mapsService);

	// Initialize routes
	logger.info('Initializing routes');
	app.use('/', apiRouter(llmController));
